...
#else
const bool enableValidationLayers = true;	//是否启用标准诊断层
#endif // NDEDUG
...
struct SwapChainSupportDetails{
	...
	std::vector<VkSurfaceFormatKHR> formats;	//图形格式
	std::vector<VkPresentModeKHR> presentModes;	//绘制模式
};
...
private:
	...
	VkPhysicalDevice physicalDevice = VK_NULL_HANDLE;	//物理设备句柄（显卡）
	VkDevice device;	//逻辑设备句柄
	VkQueue graphicsQueue;
	VkQueue presentQueue;
	VkSurfaceKHR surface;	//窗口句柄
...
void HelloTriangleApplication::createInstance() {
	...
	appInfo.sType = VK_STRUCTURE_TYPE_APPLICATION_INFO;		//结构的类型
	...
	createInfo.pApplicationInfo = &appInfo;		//Vulkan中使用结构传参，而非参数
	...
	if (vkCreateInstance(&createInfo, nullptr, &instance) != VK_SUCCESS) {
		throw std::runtime_error("faliled to create instance!");
	}
}
...
void HelloTriangleApplication::pickPhysicalDevice() {
	...
	for (const auto& device : devices) {
		if (isDeviceSuitable(device)) {
			physicalDevice = device;	//找到第一个符合要求的显卡
			break;
		}
	}
